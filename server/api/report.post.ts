...
// Add accident type and description mappings at the top level for better organization
...
// Cell mappings defined at the top level for better organization and reusability
const cellMappings = {
  // Estou section
  'estou.hrPosit': 'F8',
  'estou.ocorrN': 'O8',
...
const findTemplatePath = async (): Promise<string | null> => {
  // Improved template path resolution with multiple fallbacks
  const possiblePaths = [
    // Path 1: Standard development path
    resolve(process.cwd(), 'assets/excel/gca.xlsx'),
    
    // Path 2: Public directory path (for Vercel)
    resolve(process.cwd(), 'public/excel/gca.xlsx'),
  ];
  ...
  try {
    ...
    possiblePaths.push(
      join(currentDir, '../../../assets/excel/gca.xlsx'),
      join(currentDir, '../../assets/excel/gca.xlsx'),
      join(currentDir, '../../../public/excel/gca.xlsx')
    );
  } catch (e: any) {
  ...
  possiblePaths.push(
    '/var/task/assets/excel/gca.xlsx',
    '/var/task/public/excel/gca.xlsx'
  );
...
// Helper function to load template from URL
const loadTemplateFromUrl = async (event: any): Promise<Buffer> => {
...
const convertToPdf = (tempExcelPath: string, apiKey: string): Promise<any> => {
  ...
  try {
    ...
    try {
      ...
      if (templatePath) {
        ...
        const buffer = await loadTemplateFromUrl(event);
    ...
    // Process accident type and description
    try {
    ...
    try {
      ...
      const pdfData = await convertToPdf(tempExcelPath, CLOUDMERSIVE_API_KEY);
